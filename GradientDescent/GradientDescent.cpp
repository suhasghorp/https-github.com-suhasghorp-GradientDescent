...
std::vector<var> convertVector(std::vector<double> point) {
	std::vector<var> varPoint;
	for (double d : point) {
		varPoint.push_back(var(d));
	}
	return varPoint;
}
...
void GradientDescent::printResults(std::pair< std::vector<double>, double> results) {
...
std::vector<double> GradientDescent::computeGradientVectorAAD() {
	std::vector<double> gradientVector;
	if (m_nDims > 3)
		throw std::runtime_error(std::string("We support only 3 dimensions: f(x,y,z) and that's it..."));
	if (m_nDims == 1) {
